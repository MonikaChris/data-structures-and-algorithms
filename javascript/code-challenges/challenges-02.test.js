...
const raisedToTheThird = (arr) => {
  let cubedArr = [];
  arr.forEach(elem => cubedArr.push(Math.pow(elem, 3)));
  return cubedArr;
};
...
const addQuestion = (arr) => arr.map(elem => `${elem}?`);
...
const forEachTwoToThe = (arr) => {
  let newArr = [];
  arr.forEach(elem => newArr.push(Math.pow(2,elem)));
  return newArr;
};
...
const mapTwoToThe = (arr) => arr.map(elem => Math.pow(2, elem));
...
const charCode = (arr) => arr.map(elem => elem.charCodeAt());
...
const extractStats = (arr) => {
  return arr.map(elem => {
    let stat = {name: elem.stat.name, total: elem.effort + elem.baseStat};
    return stat;
  });
};
