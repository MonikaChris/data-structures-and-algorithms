...
const typeNum = (arr) => arr.filter(elem => !isNaN(elem));
...
const getStatName = (arr, minBaseStat) => arr.filter(stat => stat.baseStat > minBaseStat).map(elem => elem.stat.name);
...
const getCharactersWithoutChildren = (arr) => arr.filter(person => person.children === undefined);
...
const evenOddNumericValues = (arr) => arr.filter(elem => typeof elem === 'number').map(value => value % 2 === 0 ? 'even' : 'odd');
