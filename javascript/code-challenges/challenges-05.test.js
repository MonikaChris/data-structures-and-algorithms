...
const toLastNames = people => {
  const peopleList = [];
  people.map(person => peopleList.push(`${person.firstName} ${person.lastName}`));
  return peopleList;
};
...
const addPurchases = (arr) => {
  return arr.reduce((acc, curVal) => acc + curVal.purchasePrice, 0);
};
...
const reversedString = (str) => {
  return str.split('').reduce((acc, curVal) => `${curVal}${acc}`, '');
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((acc, curVal) => {
    ...
    if(curVal.children !== undefined) {
      numChildren = curVal.children.reduce(count => count +1, 0);
    }

    return numChildren + acc;
  }, 0);
};
...
const calculateAverage = (arr) => {
  const avg = arr.reduce((acc, curVal) => {
...
const extractChildren = (arr) => {
  const aChars = arr.filter(char => char.name.toLowerCase().split('').includes('a'));
  const children = aChars.reduce((acc, curVal) => {
    if(curVal.children !== undefined) {
      curVal.children.forEach(child => acc.push(child));
    }
    return acc;
  }, []);
  return children;
