...
const sortByChildren = (charArray) => {
  return charArray.sort((a,b) => {
    if(a.children.reduce(acc => acc + 1, 0) === b.children.reduce(acc => acc + 1, 0)) {
      if (a.name < b.name) {
        return -1;
      }
      else return 1;
    }
    else return (a.children.reduce(acc => acc + 1, 0) - b.children.reduce(acc => acc + 1, 0));
  });
};
...
const matchMonth = (input) => {
  return (/\b([Oo]ct)(ober)*\b/g).test(input);
};
