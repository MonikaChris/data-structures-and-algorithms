...
const maxInArray = (arr) => {
  return arr.reduce((acc, curVal) => curVal > acc ? curVal: acc);
};
...
const hasChildrenValues = (arr, character) => {
  ...
  for (let element of Object.values(char)) {
...
const houseSize = (arr) => {
  ...
  arr.forEach(object => {
    let houseObj = {
      house: object.house,
      members: Object.entries(object).reduce((acc, curVal) => {
        if ((!Array.isArray(curVal[1])) && (curVal[0] !== 'house')) {
          acc = acc + 1;
        }
        if (Array.isArray(curVal[1])) {
          acc = acc + curVal[1].length;
        }
        return acc;
      }, 0)
    };
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(object => {
    let houseObj = {
      house: object.house,
      members: Object.entries(object).reduce((acc, curVal) => {
        //Count person and spouse
        if ((!Array.isArray(curVal[1])) && (curVal[0] !== 'house')) {
          acc = acc + 1;
          //Check if spouse is deceased
          if (curVal[0] === 'spouse' && deceasedSpouses.includes(curVal[1])) {
            acc = acc - 1;
          }
        }
        //Count children
