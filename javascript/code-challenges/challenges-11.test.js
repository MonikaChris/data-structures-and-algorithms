...
function transformToLis(obj){
  return Object.entries(obj).map(pair => `<li>${pair[0]}: ${pair[1]}</li>`);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => arr.filter(elem => (typeof elem === 'number') && elem % 5 === 0)).map(arr => arr.map(num => 2**num));
};
...
let findMaleAndFemale = (data) => {
  return data.filter(char => char.gender === 'female' || char.gender === 'male').map(char => char.name).join(' and ');
};
...
let findShortest = (data) => {
  const minHeight = data.reduce((acc, curVal) => parseInt(curVal.height) < acc ? parseInt(curVal.height) : acc, 500);
  return data.filter(char => parseInt(char.height) === minHeight)[0].name;
};
