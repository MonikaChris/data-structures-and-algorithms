...
const validateEmail = (email) => {
  return /^\w+.\w+@[a-z]*(.net|.com|.org)$/g.test(email);
};
...
const validatePhoneNumber = (phoneNumber) => {
  return /^(\([0-9]{3}\))( +|)([0-9]{3})[ -]([0-9]{4})$|^([0-9]{3})[ -]?([0-9]{3})[ -]?([0-9]{4})$/g.test(phoneNumber);
};
...
const findTagNames = elements => {
  return elements.join(' ').match(/(<\/\w*>)/g).map(elem => elem.slice(1,-1));
};
